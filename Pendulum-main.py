...
def open_file_dialog():
    ...
    if in_filename != '':
        objects = read_parameters_of_pendulum_from_file(input_filename)
        """из input:
...
def menu(score):
    ...
    '''
    text('length of pendulum: x0, asperity of spring: k, amplitude of motor fluctuations: A', (min(SIZE)*0.005, min(SIZE)*0.005))
    text(str(pend.x0), (min(SIZE)*0.0005 + 120, min(SIZE)*0.0005))
    text(str(pend.k), (min(SIZE)*0.0005 + 120, min(SIZE)*0.0005))
    text(str(pend.A), (min(SIZE)*0.0005 + 120, min(SIZE)*0.0005))
...
def text(string, coords):
    ...
    '''
    text1 = pygame.font.Font(None, 38)
